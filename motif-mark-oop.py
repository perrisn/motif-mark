...
pink=[1,0.753,0.796]
red=[1,0,0]
colors_list=[orange,blue,green,pink,red]
...
def find_motif(sequence:str,motif_dict:dict):
    sequence=sequence.upper() #make everything uppercase to make it easier to search
    motif_list=[]
    motif_num=0
    for key,value in motif_dict.items():
        motif_seq=key
        motif_color=motif_color_dictionary[motif_seq] #assign a color to each motif
        ...
        for start_end in found_motif:
            ...
            motif_list.append((motif_seq,motif_start,motif_end,motif_length,motif_color))
    return(motif_list)
...
def fasta_parser(onelinefasta_file:str):
    '''This takes a fasta file where the sequence line has already been consolidated into one line and saves header
    and sequence information into a dictionary.'''
...
def find_exon(sequence):
    '''From a sequence this function will identify the exons (assuming they are denoted by uppercase characters)
        and append to a class attribute.'''
...
# Get longest line for setting up the context #
...
left_margin=25
gene_height=80
surface=cairo.SVGSurface(svg_file_name,(longest_line_length+100),((number_y*gene_height)+gene_height))
...
with open("one_line_fasta_output.fa","r") as oneline_fasta_file:
    ...
    while True:
        ...
        # Find the motifs #
        motif_list_class=[]
        motif_list_class=find_motif(sequence,known_motif_dictionary)
...
for motif in motif_color_dictionary:
    ...
    current_color=motif_color_dictionary[str(motif)]
    context.set_source_rgb(current_color[0],current_color[1],current_color[2])
    context.fill()
